...
Generates the allele clusters reference set based on the clustering from [ighvClust](#ighvclust) . The function collapse
...
------------- |----------------
`germline_distance`     |     A germline set distance matrix created by [ighvDistance](#ighvdistance) .
`germline_set`     |     A character list of the IMGT aligned IGHV allele sequences. See details for curating options.
`alleleClusterTable`     |     A data.frame of the alleles and their clusters created by [ighvClust](#ighvclust) .
