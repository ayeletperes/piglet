...
------------- |----------------
`germline_set`     |     A germline set distance matrix created by `ighvDistance` .
`mask_primer`     |     Logical (TRUE by default). If to mask with Ns the region of the primer from the germline sequence
`trimm_primer`     |     Logical (FALSE by default). If to trim the region of the primer from the germline sequence. If TRUE then, mask_primer is ignored.
