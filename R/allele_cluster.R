...
ighvClust <- function(germline_distance, family_threshold = 75, allele_cluster_threshold = 95){
  ...
  families_cut$original_allele <- rownames(families_cut)
  ...
  allele_cluster_cut$original_allele <- rownames(allele_cluster_cut)
  
  ## merge the tables
  alleleClusterTable = merge(families_cut, allele_cluster_cut, by = "original_allele")
...
generateReferenceSet <- function(germline_distance, germline_set, alleleClusterTable){
  ...
  if(!(is.matrix(germline_distance) & is.array(germline_distance))) stop("The input germline distance was not create with `ighvDistance`.")

  ### check the class of the germline set
  if(!is.character(germline_set)) stop("The input germline set is not in a character class.")
  
  ### check the class and names in alleleClusterTable
  if(!(is.data.frame(alleleClusterTable) & all(names(alleleClusterTable) %in% c("original_allele", "Family", "Allele_Cluster")))) stop("alleleClusterTable does not match the ouput from ighvClust.")
  ...
  clusters <- alleleClusterTable[!duplicated(alleleClusterTable[,c(2,3)]),c(2,3)]
  
  alleleClusterNames <- function(cluster, allele.cluster.table, germ.dist, chain, segment){
    ...
    ## get the alleles
    allele.cluster.table <- allele.cluster.table[allele.cluster.table$Family==family_cluster & allele.cluster.table$Allele_Cluster==allele_cluster,]
    
    ### check the number of alleles. Change name and stop
    
    if(nrow(allele.cluster.table)==1){
      allele.cluster.table$new_allele <- paste0(segment, "F",family_cluster, "-G", allele_cluster, "*01")
      allele.cluster.table$removed_duplicated <- F
      return(allele.cluster.table)
    }
    
    ## subset the distance matrix
    germ.dist <- germ.dist[allele.cluster.table$original_allele, allele.cluster.table$original_allele]
    diag(germ.dist) = NA
    ...
    if(length(similar)==0){
      ...
      allele.cluster.table$new_allele <- sapply(1:n, function(i){
        paste0(segment, "F",family_cluster, "-G", allele_cluster, "*", ifelse(i < 10, "0", ""), i)
      })
      allele.cluster.table$removed_duplicated <- F
      ...
      similar[, 2] <- colnames(distances[, as.numeric(similar[, 2])])
      ...
      if(nrow(similar2)>1){
        ...
        for(ii in 1:nrow(similar2)){
          ...
          vs <- similar2$V2[similar2$V1==a]
          
          ids_v1 <- which(similar2$V1 %in% vs)
          
          ids_v1_rm <- c(ids_v1_rm, ids_v1)
        }
        
        if(length(ids_v1_rm)!=0) similar3 <- similar2[-ids_v1_rm,] else similar3 <- similar2
      }else{
        similar3 <- similar2
      }
      ...
      for (ii in 1:nrow(similar3)) {
        ...
        allele_not_from_the_same_gene <- which(unlist(strsplit(alleles_idx,"[*]")[[1]]) != gene)
        if (length(allele_not_from_the_same_gene) > 1) {
          ...
          if (length(allele_not_from_the_same_gene) == 1) {
            allele_remove<- c(allele_remove, alleles_idx[[allele_not_from_the_same_gene]])
            allele_keep <-
      ...
      keep_alleles_list <- all_alleles[which(!all_alleles %in% allele_remove)]
      
      allele.cluster.table$new_allele <- ""
      for(i in seq_along(keep_alleles_list)){
        a <- keep_alleles_list[i]
        allele.cluster.table$new_allele[allele.cluster.table$original_allele==a] <- paste0(segment, "F",family_cluster, "-G", allele_cluster, "*", ifelse(i < 10, "0", ""), i)
      }
      ...
      if(length(remove_allele_list)!=0){
        for(i in seq_along(remove_allele_list)){
          ...
          allele.cluster.table$new_allele[allele.cluster.table$original_allele==a] <- allele.cluster.table$new_allele[allele.cluster.table$original_allele==a_new]
      ...
      allele.cluster.table$removed_duplicated <- allele.cluster.table$original_allele %in% allele_remove
  ...
  ## rename the alleles
  alleleClusterTable.tmp <- apply(clusters, 1,
                                   alleleClusterNames, 
  ...
  alleleClusterTable.tmp <- data.table::rbindlist(alleleClusterTable.tmp, fill = T)
  
  ## rename the germline set
  germline_set.tmp <- germline_set
  names(germline_set.tmp) <- sapply(names(germline_set.tmp), function(x){
    alleleClusterTable.tmp$new_allele[alleleClusterTable.tmp$original_allele==x]
  })
  # remove the duplicated alleles from the germline set
  germline_set.tmp <- germline_set.tmp[!duplicated(germline_set.tmp)]
  
  alleleClusterTable.tmp$thresh <- 1e-04
  
  return(list(alleleClusterTable = alleleClusterTable.tmp, 
...
#' 
#' @param    germline_set_file              The path to the germline set file, the file must be gapped by IMGT scheme for optimal results.                   
...
inferAlleleClusters <- function(germline_set_file, trim_3prime_side = 318, mask_5prime_side = 0, family_threshold = 75, allele_cluster_threshold = 95){
  
  germline_set <- tigger::readIgFasta(germline_set_file)  
...
plotAlleleCluster <- function(x, y = NULL){
  ...
  chain <- substr(alleleClusterTable$original_allele[1], 1, 3)
  segment <- paste0(chain, "V")
  imgt_allele <- setNames(alleleClusterTable$new_allele, alleleClusterTable$original_allele)
  ...
  for(i in 1:nrow(group.df)){
    ...
    allele <- group.df$A[i]
    full_allele <- db_sub$new_allele[db_sub$original_allele==allele]
...
circos.dendrogram3 <- function (dend, facing = c("outside", "inside"), max_height = NULL, 
                                use_x_attr = FALSE, sector.index = get.current.sector.index(), 
                                track.index = get.current.track.index(), lty_line = 1, fam_cut = 0.25, group_cut = 0.05) 
{
  set.seed(123456)
  ...
  dashed_line <- function(x0, x1, y0, y1, lty, lwd = 1, col){
    ...
    idx <- seq(from = x0, to = (x1-1), by = 2)
    for(x in idx){
      circos.segments(x0 = x,
                      x1 = x+1, 
                      y0 = y0,
                      y1 = y1,
                      col = col, 
  ...
  if (!is.leaf(dend)){
    ...
    dashed_line(x0 = 1, x1 = n, y0 = max_height-group_cut, 
                y1 = max_height-group_cut,
                col =   "deepskyblue4", 
                lty = lty_line, 
                lwd = 3)
    
    dashed_line(x0 = 1, x1 = n, y0 = max_height-fam_cut, 
